...
import { getSupportProjectIdea, moreButton } from '@/i18n/useNativeLocale'
    ...
    <div className={classes.ideaCard}>
      ...
      <div className={classes.ideaCard__content}>
        ...
        <div className={classes.ideaCard__tags}>
          ...
          <Link className={classes.ideaCard__tags__link} href={`/ideas/${slug}/`}>{moreButton()}</Link>
        </div>
        {onSelect && (
          <button className={classes.ideaCard__button} onClick={onSelect}>{getSupportProjectIdea()}</button>
        )}
      </div>
    </div>
  )
